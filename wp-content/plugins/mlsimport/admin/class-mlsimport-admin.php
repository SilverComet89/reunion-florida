...
class Mlsimport_Admin {
	...
	public function mlsimport_hidden_fields() {
		...
		foreach ( $options['mls-fields-admin'] as $key => $value ) {
			if ( 1 === intval($options['mls-fields-admin'][ $key ] ) ) {
				...
				print esc_html('<strong>'.$key.':</strong>'.get_post_meta( $post->ID, $meta_key, true ) . '</br>');
			}
		}
		?>
					...
					<?php if ($field['type'] === 'select'): ?>
						...
						<select class="mlsimport-select" id="<?php echo esc_attr($name); ?>" name="<?php echo esc_attr($name); ?>" <?php echo esc_attr($multiple); ?>>
							<?php foreach ($field['values'] as $select_key): ?>
								<?php if ('' !== $select_key): ?>
									<option value="<?php echo esc_attr($select_key); ?>" <?php if (is_array($value) ? in_array($select_key, $value) : $select_key === $value) echo 'selected'; ?>><?php echo esc_html($select_key); ?></option>
								<?php endif; ?>
							<?php endforeach; ?>
						</select>
	...
	*  Do api Listing Requests
	 ...
	 */
	public function mlsimport_background_process_per_item_function_old( $input_arg ) {
		mlsimport_saas_single_write_import_custom_logs( 'In processing function   ->' . wp_json_encode( $input_arg['item_id_array'] ) . PHP_EOL );
		...
		$mlsimport_item_option_data['mlsimport_item_standardstatus']  = get_post_meta( $mlsimportItemId, 'mlsimport_item_standardstatus', true );
		$mlsimport_item_option_data['mlsimport_item_property_user']   = get_post_meta( $mlsimportItemId, 'mlsimport_item_property_user', true );
		$mlsimport_item_option_data['mlsimport_item_agent']           = get_post_meta( $mlsimportItemId, 'mlsimport_item_agent', true );
		$mlsimport_item_option_data['mlsimport_item_property_status'] = get_post_meta( $mlsimportItemId, 'mlsimport_item_property_status', true );
		...
		foreach ( $attachments_to_move as $key => $import_arguments ) {
			...
			if ( 'no'  === $status ) {
				...
				$log = PHP_EOL . 'Parsing import batch  : ' . ( $key + 1 ) . ' of ' . $total_batches . '. Memory used: ' . $mem_usage_show . ' mb = ' . wp_json_encode( $import_arguments ) . PHP_EOL;
				print esc_html($log);
