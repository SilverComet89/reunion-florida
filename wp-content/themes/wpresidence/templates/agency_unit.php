...
if($user_role==3 || get_post_type($post->ID)=='estate_agency' ){
    ...
    $agent_tiktok       =   esc_html( get_post_meta($post->ID, 'agent_tiktok', true) );
    $agent_telegram     =   esc_html( get_post_meta($post->ID, 'agent_telegram', true) );
    $agent_vimeo        =   esc_html( get_post_meta($post->ID, 'agent_vimeo', true) ); 
...
<div class="agency_unit" data-link="<?php print esc_attr($link);?>">
    ...
    <div class="agency_unit_wrapper">
            ...
            print wpestate_return_agent_share_social_icons($realtor_details,'social-wrapper',''); 
            ?>
