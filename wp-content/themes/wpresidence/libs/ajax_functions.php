       ...
       // check_ajax_referer( 'wpestate_ajax_filtering', 'security' );
        ...
        $page                                       =   intval($_POST['page']);
         ...
         $answer= array(    'success'=>true,
                            'html'=>wpestate_display_listings_sh($page,  $shortcode_arguments)
                        );
...
function wpestate_display_listings_sh($page,$shortcode_arguments){
    ...
    if ($shortcode_arguments['type'] == 'agents') {
       

        $col_org= wpestate_shortocode_return_column($shortcode_arguments['row_number']);
        if( isset($shortcode_arguments['display_grid']) && $shortcode_arguments['display_grid'] =='yes' ){
            $col_org='';
        }
...
add_action( 'wp_ajax_wpestate_update_menu_bar_mobile', 'wpestate_update_menu_bar_mobile' );

if( !function_exists('wpestate_update_menu_bar_mobile') ):
    function wpestate_update_menu_bar_mobile(){
        global $wpestate_global_payments;
      //  wp_verify_nonce($_POST['security'],'wpestate_ajax_filtering');
        $user_id    = intval ( $_POST['newuser'] );
        ...
        if ($user_id!=0 && $user_id!=''){
            ...
            ob_start();
            print'<ul class="  mobile_user_menu mobilex-menu " role="menu" aria-labelledby="user_menu_trigger"> ';
...
if( !function_exists('wpestate_custom_adv_ajax_filter_listings_search') ):
    ...
    function wpestate_custom_adv_ajax_filter_listings_search(){
       ...
       // wp_reset_postdata();
        ...
        echo json_encode( array('sent'=>true,'search_meta'=>$search_meta,'saved_search'=>$saved_search, 'args'=>$args,'cards'=>$cards,'no_founs'=> $prop_selection->found_posts.' '.esc_html__('Listings','wpresidence') ) );
        die();
  }
